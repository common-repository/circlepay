...
class CirclePay_Methods_Order_Handler{
	...
	public function create_invoice()
	{
		...
		if(
			...
			update_post_meta( $this->order_id , 'circlepay_invoice_number', $this->invoice_number );
			return true;
		}
		...
		wc_add_notice( $this->connection->error_full_message( $error ) , 'error' );
	}
	...
	public function pay_invoice()
	{
		...
		if(
			isset( $response[ 'data' ] ) && 
			isset( $response[ 'data' ][0] ) &&  
			isset( $response[ 'data' ][0][ 'transaction_id' ] ) &&  
			! empty( $response[ 'data' ][0][ 'transaction_id' ] )
		){
	...
	/**
	 * Set the order status if completed successfully
	 *
	 * @access  public
	 * @since   1.0.0
	 */
	private function set_order_success_status()
	{
		$circlepay_data 		= get_option( 'woocommerce_circlepay_settings' );
		$success_status 		= sanitize_key( $circlepay_data[ 'if_paid_successfully' ] );

		$this->order->update_status( $success_status );
	 ...
	 * @since   1.0.0
	 */
	private function set_order_fail_status()
	{
