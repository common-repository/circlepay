...
class WC_Gateway_CirclePay extends WC_Payment_Gateway {
	...
	/**
	 * Suitable statuses if the payment is completed successfully
	 * @access	public
	 * @since	1.1.0
	 * @return	Array of the statuses
	 */
	public function if_order_paid_successfully_statuses()
	{
		$order_statuses = wc_get_order_statuses();
		$not_suitable 	= array( 'wc-pending', 'wc-on-hold', 'wc-cancelled', 'wc-refunded', 'wc-failed', 'wc-checkout-draft' );

		foreach ( $not_suitable as $status ) {
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			...
			'title' => array(
				...
				'type' => 'text',
				'description' => __( 'Extra title will appear on the WooCommerce payment methods page', 'circlepay' ),
				'default' => __( 'CirclePay', 'circlepay' ),
			),
			'change_order_status_to' => array(
				'title' => __( 'Change Order Status to', 'circlepay' ),
				'type' => 'title',
			),
			'if_paid_successfully' => array(
				'title' => __( 'If paid successfully', 'circlepay' ),
				'description' => __( 'What will be the order status if paid successfully?', 'circlepay' ),
				'type' => 'select',
				'options' => $this->if_order_paid_successfully_statuses(),
